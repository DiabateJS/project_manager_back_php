...
class UserManager
{
    ...
    function isAuth($login, $pwd){
        ...
        $sql = "select * from pm_users where login = :login and password = :password";
        $dicoParam = array(
          "login" => $login,
          "password" => $pwd
        );
    ...
    {
        $resultat = Helper::createResponseObject();;
        $sql = "update pm_users set fullname = :fullname , login = :login , password = :password , email = :email , profile = :profile where id = :id";
        $dicoParam = array(
    ...
    {
        $resultat = Helper::createResponseObject();;
        $sql = "insert into pm_users (fullname, login, password, email, profile) values (:fullname, :login, :password, :email, :profile)";
        $dicoParam = array(
            "fullname" => $newUser->fullname,
            "login" => $newUser->login,
            "password" => $newUser->password,
    ...
    public function deleteUser($id){
        $resultat = Helper::createResponseObject();
        $sql = "select * from pm_tache where idUser = :idUser";
        $dicoParam = array(
            "idUser" => $id
        );
        $bdMan = new BdManager();
        ...
        if (count($res) == 0){
            $sql = "delete from pm_users where id = :idUser";
            $bdMan->executePreparedQuery($sql, $dicoParam);
            $resultat["code"] = Constants::$SUCCES_CODE;
        }else{
            $resultat["code"] = Constants::$WARNING_CODE;
